...
if ($op == 'get-password-hash') {
  $pw = get_input('pw', 'get');
  $result = get_password_hash($pw);
}

elseif ($op == 'get-token') {
  $tournamentId = get_input('tournamentId', 'get');
  $result = make_token($tournamentId);
}

elseif ($op == 'get-tournaments') {
...
elseif ($op == 'add-player') {
  ...
  if ($pool) {
    $sql = "INSERT INTO player(person_id,tournament_id,division,pool) VALUES($personId,$tournamentId,'$division','$pool')";
  }
  $result = db_query($sql);
}
...
elseif ($op == 'add-tournament') {
  $keyList = get_input('keyList', 'get');	  
  $valueList = get_input('valueList', 'get');
  $sql = "INSERT INTO tournament($keyList) VALUES ($valueList)";
  $result = db_query($sql);
}
...
elseif ($op == 'add-event') {
  $keyList = get_input('keyList', 'get');	  
  $valueList = get_input('valueList', 'get');
  $sql = "INSERT INTO event($keyList) VALUES ($valueList)";
  $result = db_query($sql);
}

elseif ($op == 'update-tournament') {
  $tournamentId = get_input('tournamentId', 'get');
  $updates = get_input('updates', 'get');
  $sql = "UPDATE tournament SET $updates WHERE id=$tournamentId";
  $result = db_query($sql, 'count');
}

elseif ($op == 'update-event') {
  $eventId = get_input('eventId', 'get');
  $updates = get_input('updates', 'get');
  $sql = "UPDATE event SET $updates WHERE id=$eventId";
  $result = db_query($sql, 'count');
}

elseif ($op == 'get-recent-players') {
  $date = get_input('date', 'get');
  $sql = "SELECT DISTINCT person.name FROM person INNER JOIN player ON player.person_id = person.id INNER JOIN tournament ON player.tournament_id = tournament.id WHERE tournament.start > '$date'";
  $result = db_query($sql);
}
